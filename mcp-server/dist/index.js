...
async function callUnity(endpoint, method, data) {
    try {
...
server.tool("create_primitive", "Create primitive with optional name.", {
    type: zod_1.z.enum(["Cube", "Sphere", "Capsule", "Cylinder", "Plane", "Quad"]),
    name: zod_1.z.string().optional().describe("Custom name. Auto-increments if duplicate."),
}, async ({ type, name }) => {
...
server.tool("set_object_transform", "Set position.", {
    name: zod_1.z.string(), x: zod_1.z.number(), y: zod_1.z.number(), z: zod_1.z.number(),
}, async ({ name, x, y, z }) => {
...
server.tool("instantiate_prefab", "Instantiate a prefab.", {
    path: zod_1.z.string().describe("Path to prefab (e.g. Assets/Prefabs/Player.prefab or Resources path)"),
...
server.tool("invoke_method", "Invoke a method on a component.", {
    ...
    parameter: zod_1.z.string().optional().describe("Optional string/int/float parameter"),
...
server.tool("create_script", "Create a new C# script.", {
    fileName: zod_1.z.string().describe("Filename including .cs extension (e.g. MyScript.cs)"),
    code: zod_1.z.string().describe("The full C# code"),
}, async ({ fileName, code }) => {
...
server.tool("check_line_of_sight", "Check if there is a clear line of sight between two points or from an object.", {
    origin: zod_1.z.string().describe("Name of the origin GameObject"),
    direction: zod_1.z.string().optional().describe("Direction vector 'x,y,z' (optional, defaults to forward)"),
    maxDistance: zod_1.z.number().optional().describe("Max distance to check"),
}, async ({ origin, direction, maxDistance }) => {
    let dir = { x: 0, y: 0, z: 0 };
    if (direction) {
        const parts = direction.split(',').map(Number);
        dir = { x: parts[0], y: parts[1], z: parts[2] };
    }
...
server.tool("check_surroundings", "Check what objects are nearby (Physics OverlapSphere).", {
    origin: zod_1.z.string().describe("Name of the origin GameObject"),
    radius: zod_1.z.number().optional().describe("Radius to check (default 5)"),
}, async ({ origin, radius }) => {
...
server.tool("get_navigation_path", "Calculate a path on the NavMesh.", {
    from: zod_1.z.string().describe("Start GameObject name"),
    to: zod_1.z.string().describe("Target GameObject name"),
}, async ({ from, to }) => {
